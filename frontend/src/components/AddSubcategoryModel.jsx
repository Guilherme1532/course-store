...
const AddSubcategoryModel = ({ close, fetchData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: subcategoryResponse } = response;
      if (subcategoryResponse.success) {
        toast.success(subcategoryResponse.message);
        close();
        fetchData();
      }
    } catch (error) {
    ...
    <section className="fixed z-40 top-0 bottom-0 left-0 right-0 bg-neutral-800 bg-opacity-60 p-4 flex flex-col items-center justify-center text-black">
      <div className="bg-white max-w-4x1 w-full p-4 rounded">
        <div className="flex items-center justify-between">
          <h1 className="font-semibold">Adicionar Subcategoria</h1>
          <button
