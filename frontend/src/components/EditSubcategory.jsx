...
const EditSubcategory = ({ data, close, fetchData }) => {
  ...
  const handleUploadSubcategoryImage = async (e) => {
    ...
    if (file) {
      ...
      const { data: imageResponse } = imageUrl;
      setSubcategoryData((prev) => {
        return {
          ...prev,
  ...
  const handleUpdateSubcategory = async (e) => {
    ...
    try {
      ...
      const { data: updateResponse } = response;
  ...
  return (
    <section className="fixed z-40 top-0 bottom-0 left-0 right-0 bg-[#000000cc] p-4 flex flex-col items-center justify-center text-black">
      <div className="bg-white max-w-4x1 w-full p-4 rounded">
        ...
        <form onSubmit={handleUpdateSubcategory} className="my-3 grid gap-2">
          <div className="grid gap-1">
            <label id="categoryName">Nome</label>
            <input
              type="text"
              id="categoryName"
              placeholder="Digite um nome para a subcategoria"
